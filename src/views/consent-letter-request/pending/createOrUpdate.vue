<template>
  <div class="createItemContainer">
      ...
      <div class="dialogFullscreen__content">
        <el-form ref="model" :model="model" label-position="top">
          <el-row :gutter="16">
            ...
            <el-col :span="6">
              <el-form-item :label="$t('Lý do từ chối gửi yêu cầu CCTT')">
                <el-input
                  v-model="updateModel.rejectReason"
                  type="textarea"
                  :rows="5"
                />
              </el-form-item>
    ...
    <el-dialog
      ...
      width="30%"
    >
      <el-form ref="ruleRejectedForm" :model="updateModel">
  ...
  methods: {
    ...
    approve(isSendSMS) {
          ...
          .then(() => {
            ...
            this.updateModel.Status = ConsentLetterStatus.Approved;
            this.updateModel.IsSendLetter = isSendSMS;
            this.isLoading = true;
            updateConsentLetterRequestStatus(this.updateModel)
              .then((response) => {
    ...
    submitRejectedAccount() {
      this.$refs.ruleRejectedForm.validate((valid) => {
        if (valid) {
          this.updateModel.Id = this.model.id;
          this.updateModel.Status = ConsentLetterStatus.Rejected;
          this.isLoading = true;
          updateConsentLetterRequestStatus(this.updateModel)
            .then((response) => {
              this.$emit("reloadData");
          ...
          this.$message({
            type: "error",
            message: this.$t("Vui lòng nhập lý do từ chối gửi yêu cầu CCTT"),
            showClose: true,
          });
        }
      });
