  ...
  <div class="card">
    <div class="card-header">
      ...
      >
        <h3 class="mb-3 card-title">
          <span class="card-label">{{ $t("Banner") }}</span>
        </h3>
        <div
    ...
    >
      ...
      >
        <div class="m-2">
          <el-select
            v-model="filter.bannerType"
            class="filter-item"
            clearable
            filterable
            :placeholder="$t('Tìm theo loại banner')"
          >
            <el-option
    ...
    <div class="p-0 pb-5 card-body">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <a :href="scope.row.urlRef" target="_blank">{{
              scope.row.urlRef
            }}</a>
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('Tài liệu PDF')"
          min-width="350px"
          prop="attachmentUrl"
        >
          <template slot-scope="scope">
            <a
              :href="`${storageUrl}/userdocument/${scope.row.attachmentUrl}`"
              target="_blank"
              >{{ scope.row.attachmentUrl }}</a
            >
          </template>
        </el-table-column>
        <el-table-column
...
<script>
import { fetchList, deleteItem } from "@/api/banner";
  ...
  computed: {
    isAllowCreate() {
      return (
        ...
        this.$store.state.user.permissions.includes(Banner.Banner_Create)
      );
    },
    isAllowEdit() {
      return (
        this.$store.state.user &&
        this.$store.state.user.permissions &&
        this.$store.state.user.permissions.includes(Banner.Banner_Edit)
      );
    },
    isAllowDelete() {
      return (
        this.$store.state.user &&
        this.$store.state.user.permissions &&
        this.$store.state.user.permissions.includes(Banner.Banner_Delete)
      );
    },
    isAllowView() {
      return (
        this.$store.state.user &&
        this.$store.state.user.permissions &&
        this.$store.state.user.permissions.includes(Banner.Banner_GetList)
      );
    }
  },
  watch: {
