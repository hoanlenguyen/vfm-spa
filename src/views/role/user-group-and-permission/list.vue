<template>
  <div class="card">
    <div class="card-header">
      ...
      >
        <h3 class="mb-3 card-title">
          <span class="card-label">{{ $t("Bộ phận") }}</span>
        </h3>
        <div
    ...
    <createOrUpdate @reloadData="reloadData" :permissions='listPermission' :isAllowEdit='isAllowEdit' :isAllowCreate='isAllowCreate'/>
  </div>
</template>

<script>
import { fetchList, deleteItem, getAllDynamicRolePermission } from "@/api/role";
import createOrUpdate from "./createOrUpdate";
import { Role } from "@/utils/app-permission.js";
export default {
  ...
  computed: {
    isAllowCreate() {
      return (
        ...
        this.$store.state.user.permissions.includes(Role.Role_Create)
      );
    },
    isAllowEdit() {
      return (
        this.$store.state.user &&
        this.$store.state.user.permissions &&
        this.$store.state.user.permissions.includes(Role.Role_Edit)
      );
    },
    isAllowDelete() {
      return (
        this.$store.state.user &&
        this.$store.state.user.permissions &&
        this.$store.state.user.permissions.includes(Role.Role_Delete)
      );
    },
    isAllowView() {
      return (
        this.$store.state.user &&
        this.$store.state.user.permissions &&
        this.$store.state.user.permissions.includes(Role.Role_GetList)
      );
    }
  },
  watch: {
  ...
  created(){this.getAllDynamicRolePermission()},
  mounted() {
    this.loadData();
  },
  methods: {
    getAllDynamicRolePermission(){
      getAllDynamicRolePermission().then(resp=>{
        this.listPermission = resp.data.result
      })
    },
    loadData() {
    ...
    handleDelete(id) {
      this.$confirm(
        `${this.$t("Bạn có chắc muốn xóa bộ phận này?")}`,
        `${this.$t("Cảnh báo")}`,
        {
          ...
          deleteItem(id)
            .then((response) => {
              if (response.data.success) {
                this.$message({
                  type: "success",
                  message: `${this.$t("Xóa bộ phận thành công")}`,
                  showClose: true,
                });
                this.reloadData();
              }
