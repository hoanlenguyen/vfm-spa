<template>
  <div class="card">
    ...
    >
      <div class="m-2">
        <el-select
          v-model="filter.isNewFlowChangeInfo"
          clearable
          :placeholder="$t('Chọn flow thay đổi tt')"
        >
          <el-option
    ...
    <div class="p-0 pb-5 card-body">
      ...
      >
        ...
        <el-table-column min-width="120px" prop="isNewFlowChangeInfo">
          <template slot="header" slot-scope="scope">
            <span class="customizedHeader">{{ $t("Flow thay đổi tt") }}</span>
          </template>
          <template slot-scope="scope">
            <el-tag
              v-if="scope.row.isNewFlowChangeInfo"
              class="tableTag"
              size="small"
              type="success"
              >{{ $t("Mới") }}
        ...
        >
          <template slot-scope="scope">
            <span>{{ getContactTime(scope.row.contactTime) }}</span>
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('Số Dskh')"
        ...
        <el-table-column
          :label="$t('Hủy/từ chối bởi')"
          align="center"
          prop="updateInfoRejectedByEmail"
          sortable="custom"
          width="200px"
        >
          <template slot-scope="scope">
            <span>{{ scope.row.updateInfoRejectedByEmail }}</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.requestChangeUserInfoTime | dateTime }}</span>
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('Kích hoạt')"
...
<script>
import { fetchList, deleteItem } from "@/api/account";
import createOrUpdate from "./createOrUpdate";
import { AccountFilterType, GenderTypes } from "@/utils/enums";
export default {
  name: "CustomerList",
  ...
  data() {
    return {
      ...
      filter: {
        rowsPerPage: 10,
        sortBy: "requestChangeUserInfoTime",
        sortDirection: "desc",
        filterKeyword: "",
        accountType: 10,
        accountFilterType: AccountFilterType.RejectedChangingInformation,
      },
      filterKeyword: "",
      genderTypes: GenderTypes,
      flowOptions: [
        { label: "Flow mới", value: true },
        { label: "Flow cũ", value: false },
