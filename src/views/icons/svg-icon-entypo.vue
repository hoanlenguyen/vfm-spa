  ...
  <div class="card">
    <div class="card-header">
      <el-row type="flex" justify="space-between" align="middle">
        <h3 class="card-title">
          <span class="card-label">Entypo Icons</span>
          <span class="card-sub-label">Entypo is a suite of {{iconList.length}} carefully crafted premium pictograms by Daniel Bruce.</span>
        </h3>
    ...
    <div class="card-body">
      <s-icon icon="-404" />
      <el-row :gutter="16">
        <el-col :xl="4" :lg="6" :md="8" :sm="12" :xs="24" v-for="item of iconList" :key="item">
          <el-button
          ...
          >
            <s-icon :icon="item" />
            <span class="icon-name ml-4">{{ item }}</span>
          </el-button>
        </el-col>
      </el-row>
    </div>
...
export default {
  ...
  mounted(){
    this.iconList = icons.entypo.icons.map(p=> 'entypo-' + p);
  },
  methods: {
    generateIconCode(symbol) {
      return `<s-icon icon="${symbol}" />`;
    },
