    ...
    >
      <template #title>
        <div class="dialogFullscreen__title">
          ...
          <el-button
            type="primary"
            @click="approveManualCreateTradingAccount"
            v-if="isAllowEdit"
            >{{ $t("Duyệt tài khoản") }}
          </el-button>
          <el-button
            type="danger"
            @click="rejectManualCreateTradingAccount"
            v-if="isAllowEdit"
            >{{ $t("Từ chối") }}
          </el-button>
        </div>
          ...
          <el-row :gutter="16">
            ...
            <el-col :span="6">
              <el-form-item :label="$t('Lý do từ chối tài khoản')">
                <el-input
                  v-model="model.rejectedReasonForRegistration"
                  type="textarea"
                  :rows="5"
                  clearable
                />
    ...
    >
      <el-form ref="ruleRejectedForm" :model="modelRejected">
        ...
        <el-form-item
          ...
          prop="rejectedReasonForRegistration"
        >
          <el-input
            v-model="modelRejected.rejectedReasonForRegistration"
...
import { getForEdit, manualCreateTradingAccount } from "@/api/account";
...
export default {
  ...
  methods: {
        ...
        .then((response) => {
          if (response.data.result) {
            ...
            if (this.model.onlineTradingDocumentUrl) {
              this.lstOnlineTradingDocumentUrl =
                this.model.onlineTradingDocumentUrl;
            }
    ...
    approveManualCreateTradingAccount() {
      this.$refs["model"].validate((valid) => {
        if (valid) {
          this.$confirm(
            ...
            this.isLoading = true;
            manualCreateTradingAccount(this.model)
              .then((response) => {
    ...
    rejectManualCreateTradingAccount() {
      this.modelRejected.rejectedReasonForRegistration =
        this.model.rejectedReasonForRegistration;
