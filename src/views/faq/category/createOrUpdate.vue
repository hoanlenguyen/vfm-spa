          ...
          <el-row id="general" :gutter="16">
            ...
            <el-col :md="6" :sm="24">
              <el-form-item prop="isActive">
                <span slot="label">
                  {{ $t('Hiển thị') }}
                  <el-tooltip
                    :content="$t('Cho phép hiển thị để danh mục FAQ của bạn sẽ được hiển thị trên website')"
                    placement="bottom"
                  >
            ...
            <el-col :md="6" :sm="24">
              ...
              >
                <span slot="label">
                  {{ $t('Thứ tự hiển thị') }}
                  <el-tooltip
                    :content="$t('Thứ tự hiển thị giúp danh mục FAQ của bạn lên top đầu trong danh sách')"
                    placement="bottom"
                  >
                    <i class="el-icon-info" />
                  </el-tooltip>
              ...
              >
                <div class="w-abs-max-250">
                  <UploadCroppedImage
                    ...
                    :folderName="storageFolder.faqCategory"
                  />
                </div>
              </el-form-item>
            </el-col>
            <el-col class="mt-5" :sm="24">
              ...
              >
                ...
                >
                  <div
                    :class="{'isInvalid c-red': invalidTranslations.includes(`language${index}`)}"
                    slot="label"
                  >
                    {{ item.language }}
                    <i
                  ...
                  >
                    <el-row :gutter="16">
                      ...
                      <el-col :md="24" :sm="24">
                        ...
                        >
                          <tinymce v-model="item.shortDescription" :height="400" />
                        </el-form-item>
                      </el-col>
                    </el-row>
                  </el-form>
...
<script>
import { getForEdit, createOrUpdate } from '@/api/faqCategory'
import pageLoading from '@/components/Loading/pageLoading'
import Tinymce from '@/components/Tinymce'
import UploadCroppedImage from '@/components/UploadCroppedImage'
import {StorageFolder} from "@/utils/enums";
export default {
      ...
      if (this.model.translations && this.model.translations.length > 0) {
        this.model.translations.forEach((item, index) => {
          this.$refs[`language${index}`][0].validate((valid) => {
            if (valid) {
              this.invalidTranslations = this.invalidTranslations.filter(item => item !== `language${index}`)
            } else {
